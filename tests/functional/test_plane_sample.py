...
def test_plane_sample():
    ...
    assert torch.allclose(sampled_plane0, torch.tensor([[1, 2], [5, 6], [9, 10]], dtype=torch.float))
    ...
    assert torch.allclose(sampled_plane1, torch.tensor([[5, 6]], dtype=torch.float))
    ...
    assert torch.allclose(sampled_plane2, torch.tensor([[0, 1, 2, 3], [8, 9, 10, 11]], dtype=torch.float))
    ...
    assert torch.allclose(
        sampled_plane3, torch.tensor([[0, 2, 3, 4, 5, 0], [0, 6, 7, 8, 9, 0]], dtype=torch.float)
    )
    ...
    assert torch.allclose(sampled_plane4, torch.tensor([[3, 4, 5, 6], [5, 6, 7, 8]], dtype=torch.float))
